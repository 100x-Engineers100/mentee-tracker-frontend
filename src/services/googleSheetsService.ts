...
class GoogleSheetsService {
  ...
  categorizeMentees(mentees: AttendanceRecord[]): AttendanceRecord[] {
    return mentees.map((mentee) => {
      const missedClasses = mentee.attendanceStatus.filter(
        (status) => !status
      ).length;
  ...
  getWeeklyAttendanceRate(weekIndex: number): number {
    ...
    const presentCount = mockMentees.filter(
      (m) => m.attendanceStatus[weekIndex]
    ).length;
  ...
  async generateWeeklySummary(
    week: "current" | "week1" | "week2" | "week3" | "week4",
    format: string
  ): Promise<string> {
